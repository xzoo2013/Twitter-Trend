...
dataFramelist=[]
M=6 # parameter to define the the length of the delay
for i in range(20):
    temp=[]
    for j in range(len(data_count[i])):
         ...
         if r_server.llen(data_name[i][j])<i+M:
                current=0 #which means the ratio must be equal to 0
                last=10
         #elif int(r_server.lrange(data_name[i][j],i+1,i+1)[0])==0:
         #       current=current+10
         #       last=10
         else:
                current=current+0.5
                last1=float(r_server.lrange(data_name[i][j],i+1,i+1)[0])+0.1
                last2=float(r_server.lrange(data_name[i][j],i+2,i+2)[0])+0.1
                last3=float(r_server.lrange(data_name[i][j],i+3,i+3)[0])+0.1
                last4=float(r_server.lrange(data_name[i][j],i+4,i+4)[0])+0.1
                last5=float(r_server.lrange(data_name[i][j],i+5,i+5)[0])+0.1
                last=0.1*last1+0.15*last2+0.2*last3+0.25*last4+0.3*last5
         ratio=current/last  
...
writer = ExcelWriter('outputbigramsRatio2.xlsx')
for i in range(20):
